...
import { toggleFavorite } from '../service/song.api'
import './player.scss'
...
const Player = () => {
    const { song, handleNextSong } = useSong()
    ...
    useEffect(() => {
        ...
        setIsFavorite(Boolean(song && favoriteIds.includes(song._id)))
    }, [song, user])
    ...
    const handleToggleFavorite = async () => {
        if (!song) return
        try {
            const data = await toggleFavorite(song._id)
            const updatedFavorites = data.favorites
            setIsFavorite(updatedFavorites.some((favorite) => favorite._id === song._id || favorite === song._id))
            if (user) {
            ...
            <div className="player__info">
                ...
                <div className="player__meta">
                    ...
                    <div className="player__meta-row">
                        ...
                        <button className={`player__fav ${isFavorite ? 'favorite' : ''}`} onClick={handleToggleFavorite} title="Toggle favorite">
                            {isFavorite ? '♥' : '♡'}
                        </button>
                    </div>
                </div>
